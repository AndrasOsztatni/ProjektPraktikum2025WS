...
end=binarySearch(timestamp_ms, 17502)
...
plt.title("Pressure change in time (30mm 45° 1.0)")
plt.ylabel("Pressure (kPa)")
plt.xlabel("Time (ms)")
plt.grid()
plt.savefig(os.path.join(script_dir, "30mm_in45_1.0_p(t).pdf") , format="pdf")
plt.clf()
...
plt.grid()
plt.title("Frequency spectrum of the \nPressure oscillation (30mm 45° 1.0)")
plt.tight_layout()
plt.savefig(os.path.join(script_dir, "30mm_in45_1.0_fft.pdf"), format="pdf")
plt.clf()
...
plt.grid()
plt.tight_layout()
plt.savefig(os.path.join(script_dir, "30mm_in45_1.0_powspec.pdf"), format="pdf")
plt.clf()
